...
class ResumeService:
    ...
    async def upload_resume(
        ...
        *,
        ...
        existing_resume = self.resumes.get_primary_by_user(current_user.id)

        if existing_resume:
            self.resumes.delete_file(existing_resume)
            self.resumes.delete(existing_resume)
    ...
    def delete_resume(
        ...
        self.resumes.delete_file(resume)
        self.resumes.delete(resume)

        if resume.student_profile_id:
            from app.services.student_service import StudentService

            StudentService(self.db)._update_completion_score(
                resume.student_profile_id
            )

    @staticmethod
    def _authorize_resume_read(
        current_user: User,
        resume: Resume,
    ) -> None:
        privileged_roles = {
        ...
        if (
            current_user.id == resume.user_id
            or current_user.role in privileged_roles
        ):
            return
